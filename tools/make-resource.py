...
parser.add_argument(
    "-output", type=str, help="output qbtheme file", default="style.qbtheme"
)
...
parser.add_argument(
    ...
    dest="iconsDir",
    default="",
    help="directory which contains custom icons",
)
parser.add_argument(
    "-dir-prefix",
    type=str,
    default="",
    dest="dirPrefix",
    help="prefix added to all files",
)
parser.add_argument(
    "-config", type=str, dest="config", default=None, help="file used as config.json"
)
parser.add_argument(
    "-find-files",
    action="store_true",
...
parser.add_argument(
    ...
    nargs="*",
    default=["*"],
    help="files to include in resources from baseDir, supports glob patterns",
)
...
IconFiles = (
    ...
    else [f for f in glob.glob(os.path.join(args.iconsDir, "*"))]
)
print(IconFiles)
...
with open("resources.qrc", "w") as rcc:
    ...
    rcc.write("\t<qresource>\n")
    rcc.write(
        "\t\t<file alias='stylesheet.qss'>%s</file>\n"
        % (os.path.join(args.baseDir, args.style))
    )
    rcc.writelines(
        [
            "\t\t<file alias='icons\\%s'>%s</file>\n" % (os.path.split(x)[1], x)
            for x in IconFiles
        ]
    )
    if config_file:
...
cmd = [
    os.path.join(os.path.dirname(os.path.realpath(__file__)), "rcc"),
    "-binary",
    "-o",
    args.output,
    "resources.qrc",
